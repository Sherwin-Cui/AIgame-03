...
export class GameEngine {
    ...
    createCheckEventHandler(eventName) {
        return (gameState) => {
            ...
            const checkResult = this.stateManager.performCheck(
                eventData.checkType,
                difficulty,
                eventData.itemBonuses || []
            );
            ...
            return {
                ...
                baseSuccessRate: eventData.baseSuccessRate,
    ...
    async useItem(itemName) {
        try {
            ...
            this.stateManager.addDialogue(
                '系统',
                `使用了道具：${this.getItemDisplayName(itemName)}`,
                'system'
            );
    ...
    // 检查道具可用性
    checkItemAvailability(itemName, gameState) {
        switch (itemName) {
            case 'xuande-brush':
                if (!gameState.hasXuanDeBrush) {
                    return {
                        available: false,
                        reason: '玄德亲笔已经使用过了'
                    };
